...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            logger.info("Failed to process jwt authentication request", ex);
    ...
    private void processJwtAuthentication(HttpServletRequest request) throws AuthenticationException{
        ...
        if (StringUtils.isBlank(authHeader) || !authHeader.startsWith("Bearer ")) {
            log.info("Request without jwt header");
            return;
        }
        ...
        if (jwtVerifyResult.isVerified()) {
            ...
            log.info("User(uuid={}) fail to authenticate : expired token", jwtVerifyResult.getSubject());
        } else {
